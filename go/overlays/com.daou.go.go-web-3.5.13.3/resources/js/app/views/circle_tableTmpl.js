define('views/circle_tableTmpl', function(require){
    ...
    var CompanyListView = Backbone.View.extend({
        ...
        _renderTemplate: function(companies) {
            if (_.isArray(this.ids)) {
                companies = _.filter(companies, function(company) {
                    return _.contains(this.ids, ('' + company['id']));
                }, this);
            }
            ...
            var tmpl = [
                ...
                '</span>',
                '<select id="circle_company_select">',
                '    {{#companies}}',
                '    <option value="{{name}}" data-id="{{id}}" data-name="{{name}}">{{name}}</option>',
                '    {{/companies}}',
                '    {{^companies}}',
        ...
        _onCompanySelected: function(e) {
            this._notifyCompanySelected();
        },

        _notifyCompanySelected: function() {
            var $selected = $('#circle_company_select > option:selected');
            console.log('onCompanySelected: ' + $selected.attr('data-id'));
            this.observer.trigger('companySelected', $selected.attr('data-id'), $selected.attr('data-name'));
        }
    })
    ...
    var SelectedNodeListView = Backbone.View.extend({
        ...
        initialize: function(options) {
            ...
            this.isExceptionList = (_.isBoolean(options.isExceptionList)) ? options.isExceptionList : false;
            this.isUseActionMenu = (_.isBoolean(options.isUseActionMenu)) ? options.isUseActionMenu : !this.isExceptionList;
        },
        ...
        render: function() {
            ...
            this.$el.on('click', 'input[name=enroll_permission]', $.proxy(this._toggleEnrollPermission, this));
            this.$el.on('click', 'input[name=remove_permission]', $.proxy(this._toggleRemovePermission, this));
        },
        ...
        _renderNodeList: function() {
            this.$el.html(this._makeTemplate({
                ...
                'isUseActionMenu' : this.isUseActionMenu,
                'isExceptionList' : this.isExceptionList,
                'lang' : {
                	"accessTarget" : adminLang["열람자"],
                	"exceptionTarget" : adminLang["예외자"],
                	"includeSubDept" : commonLang["하위부서포함"],
                	"writable" : adminLang["등록권한"],
                	"removable" : adminLang["삭제권한"],
                    "remove" : commonLang["삭제"]
                }
            }));
        },
        ...
        _makeTemplate: function(data) {
            var tmpl = [
                ...
                '   <thead>',
                '       <tr>',
                '           <th>{{#isExceptionList}}{{lang.exceptionTarget}}{{/isExceptionList}}{{^isExceptionList}}{{lang.accessTarget}}{{/isExceptionList}}</th>',
                '           <th>{{lang.includeSubDept}}</th>',
                '           {{#isUseActionMenu}}',
                '           <th>{{lang.writable}}</th>',
                '           <th>{{lang.removable}}</th>',
                '           {{/isUseActionMenu}}',
                '           <th>{{lang.remove}}</th>',
                '       </tr>',
                '    </thead>',
                '    <tbody class="vertical_wrap_s">',
                '       {{#nodes}}',
                '       <tr {{#isFirst?}}class="first"{{/isFirst?}} data-nodeId="{{nodeId}}" data-nodeType="{{nodeType}}" data-nodeDeptId="{{nodeDeptId}}" data-actions="{{actions}}">',
                '           <td class="align_c">',
                '               <span class="minor"> {{nodeTypeLabel}} {{#nodeValue}}:{{/nodeValue}} {{nodeValue}}{{#visibleCompanyName}}{{#nodeCompanyName}} - {{nodeCompanyName}}{{/nodeCompanyName}}{{/visibleCompanyName}}',
                '           </td>',
                '           <td class="align_c">',
                '               {{^withoutSubDept}}{{#isDeptOrSubDeptType?}}',
                '               <span class="sub_depart"> <input type="checkbox" id="include_sub_dept" name="include_sub_dept" {{#isSubDeptType?}}checked{{/isSubDeptType?}}> </span>',
                '               {{/isDeptOrSubDeptType?}}{{/withoutSubDept}}',
                '           </td>',
                '           {{#isUseActionMenu}}',
                '           <td class="align_c">',
                '              <span><input type="checkbox" id="enroll_permission" name="enroll_permission" {{#writable}}checked{{/writable}}></span>',
                '           </td>',
                '           <td class="align_c">',
                '              <span><input type="checkbox" id="remove_permission" name="remove_permission" {{#removable}}checked{{/removable}}></span>',
                '           </td>',
                '           {{/isUseActionMenu}}',
                '           <td class="align_c">',
                '              <span class="btn_border"><span class="ic ic_delete" title="{{delete_label}}"></span></span>',
                '           </td>',
                '       </tr>',
                '       {{/nodes}}',
                '    </tbody>',
        ...
        _makeTemplateData: function(collection) {
            return _.map(collection.models, function(m, idx) {
                return {
                    ...
                    'writable' : m.get('actions').indexOf(ACTION.WRITABLE) >= 0,
                    'removable' : m.get('actions').indexOf(ACTION.REMOVABLE) >= 0,
                    'nodeId' : m.get('nodeId'),
        ...
        _toggleEnrollPermission: function(e) {
            var $target = $(e.currentTarget).parent().parent().parent(),
                isChecked = $(e.currentTarget).is(':checked'),
                model = this.collection.getByNodeIdAndType($target.attr('data-nodeId'), $target.attr('data-nodeType'), $target.attr('data-nodeDeptId'));

            var actions = model.get('actions').length > 0 ? model.get('actions').split(",") : ['read'];
            if(isChecked){
                actions.push(ACTION.WRITABLE);
            }else{
                actions.splice(actions.indexOf(ACTION.WRITABLE),1);
            }
            model.set('actions', actions.join(","));
            this._renderNodeList(); // target 이 ghost 가 되버리므로 버블링을 막거나, rendering 을 다시 하지 않아야 한다.
            e.stopPropagation();
        },

        _toggleRemovePermission: function(e) {
            var $target = $(e.currentTarget).parent().parent().parent(),
                isChecked = $(e.currentTarget).is(':checked'),
                model = this.collection.getByNodeIdAndType($target.attr('data-nodeId'), $target.attr('data-nodeType'), $target.attr('data-nodeDeptId'));
            ...
            var actions = model.get('actions').length > 0 ? model.get('actions').split(",") : ['read'];
            if(isChecked){
                actions.push(ACTION.REMOVABLE);
            }else{
                actions.splice(actions.indexOf(ACTION.REMOVABLE),1);
            }
            model.set('actions', actions.join(","));
            this._renderNodeList(); // target 이 ghost 가 되버리므로 버블링을 막거나, rendering 을 다시 하지 않아야 한다.
            e.stopPropagation();
    ...
    var NodeSelectorView = Backbone.View.extend({
            ...
            initialize: function(options) {
                ...
                this.callback = (_.isFunction(options.callback)) ? options.callback : function() { return; };
            ...
            _renderSelectableList: function(type, companyId, companyName) {
                ...
                var Selector = (_.contains(['company','user', 'department', 'subdepartment'], type)) ? OrgNodeSelectorView : ClassNodeSelectorView;
    ...
    var OrgNodeSelectorView = Backbone.View.extend({
        ...
        render: function() {
            var orgSelectCallback = function(type, callback, info) {
                ...
                if(this.useAction || !this.isExceptionList){
            ...
            var htmls = [
                ...
                '{{/useAction}}',
                '<span class="btn_wrap">',
                '    <span class="btn_s select_org_node" data-type={{type}}>{{add}}</span>',
                '</span>'
            ];
    ...
    var ClassNodeSelectorView = Backbone.View.extend({
        ...
        _renderClassNodeList: function(data, type) {
            var htmls = [
                ...
                '</span>',
                '{{/useAction}}',
                '<span class="btn_wrap class_selector_button"><span class="btn_s">{{lang.add}}</span></span>'
            ];
     ...
     *
     */
    var CircleViewWithTableTmpl = Backbone.View.extend({
        ...
        initialize: function(options) {
            ...
            if (_.isEmpty(data.nodes)) { data.nodes = []; }
    ...
    return CircleViewWithTableTmpl;
});