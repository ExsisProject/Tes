...
define([
    ...
    "hgn!approval/templates/approval_doclist",
	"i18n!nls/commons",
...
function(
	...
	var ApprovalDocListView = BaseDocListView.extend({
		columns: {
		...
		initialize: function(options) {
			...
			this.collection = new ApprovalDocList();
			...
			if ( baseUrl && baseUrl == 'approval/doclist/approve/' + this.type ) {
				...
				this.collection.setDuration({duration : 'all'});
                ...
                "buttons" : [
                    {
                        ...
                        'callback': function(rs) {
                            ...
                            if(rs.parent().find('#allSelectTr').is(':visible') && rs.parent().find('#allSelectMsg3').attr('data-value') == 'folder'){
                                ...
                                model = new UserAllDocsCopyModel({folderType : 'userapprovefolder'});
        ...
        renderLayout: function() {
			this.$el.html(ApprovalDocListTpl({
				lang: lang
			}));
			...
			this.contentTop.setTitle(approvalLang['결재 문서함']);
        ...
        makeCompleteDocumentCountingUrl: function(){
            var url = GO.contextRoot + "api/approval/doclist/approve/complete/count?";
            return url + this.makeParams();
        }
	});
	
	return ApprovalDocListView;
});