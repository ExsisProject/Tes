...
define([
    ...
    "hgn!approval/templates/viewer_doclist",
...
function(
	...
	var ViewerDocListView = BaseDocListView.extend({
		...
		initialize: function(options) {
			...
			this.collection = new ViewerDocList();
			...
			if ( baseUrl && baseUrl == 'approval/doclist/viewer/' + this.type ) {
                ...
                "buttons" : [
                    {
                        ...
                        'callback': function(rs) {
							...
							if(rs.parent().find('#allSelectTr').is(':visible') && rs.parent().find('#allSelectMsg3').attr('data-value') == 'folder'){
                                var documentType = self.collection.type + "documents";
                                model = new UserAllDocsCopyModel({folderType : 'userviewerfolder', documentType : documentType});
        ...
        renderLayout: function() {
			...
			// 문서함 툴바
//			this.docToolBar = new DocToolBarView;
//			this.docToolBar.render();
//			this.$el.find('div#docToolbar').append(this.docToolBar.el);
//			this.docToolBar.setDeptFolderMode();

			this.contentTop.setTitle(approvalLang['참조/열람문서함']);
		...
		makeCompleteDocumentCountingUrl: function(){
            var folderType = this.collection.type == "all" ? "viewer" : this.collection.type;
            var url = GO.contextRoot + "api/approval/doclist/"+folderType+"/complete/count?";
			return url + this.makeParams();
		}
	});

	return ViewerDocListView;
});