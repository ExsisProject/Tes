    ...
    function(
        ...
        ApprFormSearchTreeView = Backbone.View.extend({
            ...
            initialize: function(options) {
                ...
                if (_.isNumber(this.options.inputMaxLength)) { this.inputMaxLength = this.options.inputMaxLength; }
            ...
            _loadSearchList: function(keyword) {
                ...
                folderTpl = Hogan.compile([
                    '<li class="jstree-leaf" data-id="{{id}}" data-title="{{name}}" data-rel="{{state}}">',
                    '   <a  data-bypass >',
                    '       <ins class="jstree-icon appr_form">&nbsp;</ins>{{name}}',
                    '   </a>',
                    '</li>'
                ].join(''));
                ...
                $.ajax({
                    url : GO.contextRoot + 'api/approval/apprform/tree/search',
                    type : 'get',
            ...
            _renderTemplate: function() {
            	var lang = {
            			placeholder : approvalLang['양식제목']
            	};
            	
                var htmls = [
                    '<form onsubmit="return false;"><section class="search">',
                    '    <div class="search_wrap">',
                    '        <input id="' + this.searchInputId + '" class="search" maxlength="'+this.inputMaxLength+'"  type="text" placeholder="{{lang.placeholder}}">',
                    ...
                    '<div id="' + this.treeElId + '" class="jstree jstree-0 jstree-focused jstree-default">',
                    '</div>',
                    '<div id="' + this.searchResultElId + '"  style="min-height: ' + this.css.minHeight + 'px; max-height: ' + this.css.maxHeight + 'px; display: none;">',
            ...
            _onLoadNode: function( node , success_callback , error_callback ) {
                ...
                this.treeEl.find('a[rel="FORM"] > ins').addClass('appr_form');
/*                this.treeEl.find('li').each(function(k, v){
                	$(v).attr('rel', $(v).find('a').first().attr('rel'))
                })*/
            },
            ...
            _processSearchData : function(selectedObj) {
                ...
                data = {
                    ...
                    'type' : (selectedObj.data('rel') == 'NORMAL')? 'FORM' : null 
                };
