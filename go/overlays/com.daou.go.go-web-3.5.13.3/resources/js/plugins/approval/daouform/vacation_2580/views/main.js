define(function(require) {
    ...
    var NonHolidayCountModel = require('approval/daouform/vacation_2580/models/non_holiday_count');

    var typeTpl = require('hgn!approval/daouform/vacation_2580/templates/typeTemplate');
    var halfTpl = require('hgn!approval/daouform/vacation_2580/templates/halfTemplate');
    var pointTpl = require('hgn!approval/daouform/vacation_2580/templates/pointTemplate');
    ...
    var Vacation = Backbone.View.extend({
        ...
        render : function() {
            ...
            makePointEl(this.$selectedDaysAreaEl, '선택일수', 'usingPoint', 1);
            ...
            function makeHalfEl($vacationHalfEl) {
                ...
                $vacationHalfEl.html(halfTpl({
                    checkStartEnd : '{{check_시작일_종료일}}',
                    checkQuarter : '{{check_오전前_오전後_오후前_오후後}}'
                }));
            }
        ...
        getVacationType: function(vacationName) {
            return this.vacationTypes.filter(function (value) {
                return value.name == vacationName;
            })[0];
        },
        renderEditDocument : function() {
            ...
            this.$vacationHalfAreaEl.find("input[name^='checkQuarter']").unwrap('span'); //임시저장시 wrapping된 span 제거 - nextUntil 메소드 사용을 위해
            this._eventBind();
        ...
        _eventBind : function() {
            ...
            this.$vacationHalfAreaEl.find('input:checkbox').filter("input[name^='checkStartEnd']").on('change', function(e){
                ...
                if($target.is(':checked')) {
                    if(!isStartEndSameDate() && $target.attr('id') == "startHalf") {
                        ...
                        $target.closest('span.halfArea').find("input#startQuarter4").prop('checked', true);
                    } else { // 기본 오전前(Quarter1) 자동 클릭
                        $target.closest('span.halfArea').find("input[id$='Quarter1']").prop('checked', true);
                    }
            ...
            var $startQuarterArea = this.$vacationHalfAreaEl.find('input:checkbox')
                .filter("input[name^='checkQuarter']").filter("input[name$='startHalf']");
            $startQuarterArea.on('change', function(e){
                var $target = $(e.currentTarget);
                ...
                if (!isStartEndSameDate()) {
                    ...
                    if ($target.attr('id') !== 'startQuarter4') {
                ...
                var checkedCnt = $startQuarterArea.filter(':checked').length;
                if (checkedCnt > 1) {
                    fillInBetweenChecked($startQuarterArea.filter(':checked'));
                }
                // 체크 하나라도 있으면 시작일 체크
            ...
            var $endQuarterArea = this.$vacationHalfAreaEl.find('input:checkbox')
                .filter("input[name^='checkQuarter']").filter("input[name$='endHalf']");
            $endQuarterArea.on('change', function(e){
                var $target = $(e.currentTarget);
                ...
                var checkedCnt = $endQuarterArea.filter(':checked').length;
                if (checkedCnt > 1) {
                    fillInBetweenChecked($endQuarterArea.filter(':checked'));
                }
                // 체크 하나라도 있으면 종료일 체크
           ...
           function showContinuousQuarterSuggestionSlide() {
               $.goError('(반)반차는 연이어서만 선택할 수 있습니다. (분리해서 사용하려면 나눠서 신청해주세요)');
           }
        ...
        refresh : function() {
            var usingPoint = getUsingPoint(this.$startDateEl, this.$endDateEl, this.getVacationName(), this.drafterId);
            ...
            this.validate().applyPoint(this.applyPoint, selectedDays);
            ...
            function getApplyPoint($halfEl, usingPoint) {
                var quarterPoint = 0.25;
                var checkedStartEndCnt = $halfEl.find("input[name^='checkStartEnd']").filter(":checked").length;
                var checkedQuarterCnt = $halfEl.find("input[name^='checkQuarter']").filter(":checked").length;

                return usingPoint - checkedStartEndCnt + (quarterPoint * checkedQuarterCnt);
            }

            function getUsingPoint($startDateEl, $endDateEl, vacationName, drafterId){
        ...
        getVariablesData : function() {
            return {
                ...
                startQuarter1 : $('#startQuarter1').is(':checked'),
                startQuarter2 : $('#startQuarter2').is(':checked'),
                startQuarter3 : $('#startQuarter3').is(':checked'),
                startQuarter4 : $('#startQuarter4').is(':checked'),
                endQuarter1 : $('#endQuarter1').is(':checked'),
                endQuarter2 : $('#endQuarter2').is(':checked'),
                endQuarter3 : $('#endQuarter3').is(':checked'),
                endQuarter4 : $('#endQuarter4').is(':checked'),
