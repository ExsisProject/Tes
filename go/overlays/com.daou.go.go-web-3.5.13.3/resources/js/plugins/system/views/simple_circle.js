/***
 * 시스템 관리 어드민에서만 쓰이는 circle. 기존 circle과는 companyCollection만 틀림. 
 * offset이 400개만 넘어가면 기존 collection.fetch할떄
 * api가 30초가 넘어서 fail되기 때문에 새로운 api를 호출할 collection을 만듬.
 * 기존꺼를 수정해서 할수도 있지만 일단 영향도를 최소화 하기 위해 새로 파일을 만들었습니다. 
 * 
 */
(function() {
    ...
    function(
        ...
        */
        ...
        var CompanyListView = Backbone.View.extend({
            ...
            render: function() {
                this.$el.append(this._renderTemplate(this.collection.toJSON()));
        ...
        var SelectedNodeListView = Backbone.View.extend({
            ...
            _makeTemplate: function(data) {
                var tmpl = [
                    ...
                    '                <span class="minor">',
                    '                    {{nodeTypeLabel}} : {{nodeValue}}{{#visibleCompanyName}}{{#nodeCompanyName}} - {{nodeCompanyName}}{{/nodeCompanyName}}{{/visibleCompanyName}}',
