(function() {
    ...
    define([
        ...
        "app",
        "helpers/form",
        "system/collections/simple_companies",
        ...
        */
        ...
        var CompanyListView = Backbone.View.extend({
            ...
            events: {
                ...
                'click span[data-type="add"]' : '_onClickAdd'
            },
            ...
            render: function() {
                ...
                this.collection.fetch({
                    success: function(collection) {
                        ctx.$el.append(ctx._renderTemplate(collection.toJSON(), ctx.useAction));
                        ctx._notifyCompanySelected();
                    }
                });
            },
            
            _renderTemplate: function(companies, useAction) {
                ...
                var tmpl = [
                    ...
                    '</select>',
                    '{{#useAction}}',
                    '<span class="option_wrap" id="publicWritableAuthWrap">',
                    '    <input type="checkbox" value="" class="writePossible" id="publicWritableAuth">',
                    '    <label for="publicWritableAuth">{{lang.writable}}</label>',
                    '</span>',
                    '{{/useAction}}',
                    '<span class="btn_wrap">',
                    '    <span class="btn_s select_org_node" data-type="add">{{add}}</span>',
                    '</span>'
                ];
            ...
            _onClickAdd : function(e){
            	var nodeModel = new CircleNodeModel();
                var $selected = this.$el.find('#circle_company_select > option:selected');
                var actions = [];
                if(this.useAction){
                    actions.push(ACTION.READABLE);
                    if(this.$el.find('#publicWritableAuth').prop('checked')){
                    	actions.push(ACTION.WRITABLE);
                    }
                }
                nodeModel.set('nodeCompanyId', $selected.attr('data-id'));
                nodeModel.set('nodeId', $selected.attr('data-id'));
                nodeModel.set('nodeCompanyName', $selected.attr('data-name'));
                nodeModel.set('actions', actions);
                nodeModel.set('nodeType', 'company');
        ...
        var SelectedNodeListView = Backbone.View.extend({
            ...
            _makeTemplate: function(data) {
                var tmpl = [
                    '<div class="option_display" style="height: 170px;overflow-y: auto;overflow-x: hidden;">',
                    '    <div class="vertical_wrap_s">',
                    '        <ul class="list_option">',
                    '            {{#nodes}}',
                    '            <li {{#isFirst?}}class="first"{{/isFirst?}} data-nodeId="{{nodeId}}" data-nodeType="{{nodeType}}" data-nodeDeptId="{{nodeDeptId}}" data-actions="{{actions}}">',
                    '                <span class="minor">',
                    '                    {{nodeTypeLabel}} : {{nodeValue}}{{#visibleCompanyName}}{{#nodeCompanyName}} {{^isCompanyType}}-{{/isCompanyType}} {{nodeCompanyName}}{{/nodeCompanyName}}{{/visibleCompanyName}}',
                    '                    {{^withoutSubDept}}{{#isDeptOrSubDeptType?}}',
                    '                        <span class="sub_depart"> ( <input type="checkbox" name="include_sub_dept" {{#isSubDeptType?}}checked{{/isSubDeptType?}}><label for="">{{including_sub_dept_label}}</label> )</span>',
        ...
        var CircleView = Backbone.View.extend({
            ...
            hideOrgSlide : function(){
            	if(this.nodePickerView.nodePickerView && this.nodePickerView.nodePickerView.orgSlide){
            		this.nodePickerView.nodePickerView.orgSlide.close();
            	}
            },
