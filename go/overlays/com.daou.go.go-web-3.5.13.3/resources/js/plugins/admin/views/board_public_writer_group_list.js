(function() {
    define([
            ...
            "backbone",
            "app",
            "hgn!admin/templates/board_public_writer_group_list",
            "admin/collections/board_public_writer_group_list",
            "i18n!nls/commons",
            ...
            var boardGroupList = Backbone.View.extend({
                ...
                addBtn : function(e){
                    ...
                    ulList = "<li data-code='"+selectGroupList.val()+"'>"+
                        ...
                        "</li>";
                    this.$el.find("#groupUl").addClass("list_option").append(ulList);
                },
                validateDuplicatePermission : function(data_code, data_type, data_permission){
                    var groupUls = this.$el.find("#groupUl li");
                    var	permissionDatas = [];
                    ...
                    if("department" == select.val() || "user" == select.val()){
                        if(e != undefined){
                            ...
                            if("department" == select.val()) {
                                $(e.currentTarget).parent().find('#sharedType').parent().parent('span').show();
                            }else{
                                $(e.currentTarget).parent().find('#sharedType').parent().parent('span').hide();
                            }
                        }else{
                            this.$el.find('.groupListSelect').hide();
                        }
                    }else{
                        if(e != undefined){
                            $(e.currentTarget).parent().find('.groupListSelect').show();
                            $(e.currentTarget).parent().find('#sharedType').parent('label').parent('span').hide();
                        }else{
                            this.$el.find('.groupListSelect').show();
                        }
                    }
                    var selectList = "";
                ...
                addCollback : function(rsData, type, deleteEl, modiEl) {
                    ...
                    if(type == "Department"){
                        if(this.$el.find("#sharedType").is(":checked")){
                            sharedType = "("+lang.low_dept+")";
                            sharedTypeVal = "TO_SUB";
                        }
                    ...
                    var ulList = "";
                    ulList = "<li data-code='"+rsData.id+"' data-type='"+ type +"' data-sharedType ='"+sharedTypeVal+"'>"+
                        "<span class='major'>["+ownerTypeLang+" : "+rsData.name+"]</span>"+
                        "<span class='minor'>"+sharedType+"</span>"+
                        "<span class='btn_border'><span class='ic ic_delete' title='"+lang.del+"' data-btntype='publicDelete'></span></span>"+
                        "</li>";
                    this.$el.find("#groupUl").addClass("list_option").append(ulList);
                    if(deleteEl != undefined){
