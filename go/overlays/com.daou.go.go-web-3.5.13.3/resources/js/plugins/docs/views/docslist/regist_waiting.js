define('docs/views/docslist/regist_waiting', function(require) {
	...
	var RegistWaitingDocsListTpl = require("hgn!docs/templates/docslist/base_docslist");
	...
	var ApproveWaitingList = BaseDocsList.extend({
		...
		url: function() {
			return '/api/docs/folder/registwaiting?' + this._makeParam();
		}
	});
	...
	var BulkDeleteList = Backbone.Model.extend({
		url : function(){
			var url = '/api/docs/requestwaiting';
			return url;
		}
	});
	...
	return BaseDocsView.extend({
        ...
        initialize: function (options) {
        	...
        	this.collection = new ApproveWaitingList({folderType : "registwaiting"});
        	this.collection.usePeriod = true;
        	
        	var baseUrl = sessionStorage.getItem('list-history-baseUrl');
        	var regexp = new RegExp('(#)?docs/folder/registwaiting$');
        ...
        render: function () {
            ...
            $("#content header h1").text(docsLang["등록 대기 문서"]);
            this.$el.html(RegistWaitingDocsListTpl({
                lang : lang,
                usePage : true,
                useToolbar : true,
                isRegistWaiting : true
            }));
            this._renderRegistWaitingList();
        ...
        _renderRegistWaitingList: function(){
        	this.checkboxColumn = {
					id : 'checkedAllRegistWaitingDocs',
					name : 'checkedAllRegistWaitingDocs'
			}
        ...
        _bulkDelete: function(e) {
	        ...
	        var bulkApproveList = new BulkDeleteList();
