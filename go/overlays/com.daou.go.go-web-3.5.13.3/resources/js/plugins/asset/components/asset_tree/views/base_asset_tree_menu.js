define('asset/components/asset_tree/views/base_asset_tree_menu', function(require) {
	...
	var BaseAssetTreeMenuView = BaseAssetTreeView.extend({
		/**
		 * 메뉴별 고유 ID
		 */
		menuId: null, 
		...
		initialize: function(options) {
			var opts = options || {};
			BaseAssetTreeView.prototype.initialize.apply(this, arguments);
		 ...
		 * @Override
		 */
		renderChildView: function(node) {
			var childView = new this.constructor({
				"nodes": this.assetTreeNodes,
		 ...
		 * 노드가 닫힌 상태인지 여부 반환
		 */
		isClosedNode: function($node) {
			return $node.find("span.btnToggleNode").hasClass("open");
		}, 
		...
		foldNode: function($node) {
			...
			$node.find('.board-tree-nodes').remove();
		...
		toggleNode: function($node) {
			...
			var treeNode = this.assetTreeNodes.get(nodeId);
			
			if(!treeNode) {
				return;
			}
			
			if(this.isClosedNode($node)) {
