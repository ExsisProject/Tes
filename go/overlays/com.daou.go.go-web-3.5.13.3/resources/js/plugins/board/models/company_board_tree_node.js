 ...
 */
define('board/models/company_board_tree_node', function(require) {
	...
	var CompanyBoardTreeNode = BoardTreeNode.Model.extend({
		urlRoot: function() {
			return GO.config('contextRoot') + (this.isAdminService() ? 'ad/': '') + 'api/company/board/' + this.get('nodeType');
		},
	...
	var CompanyBoardTreeNodes = BoardTreeNode.Collection.extend({
		...
		url: function() {
			var url = GO.config('contextRoot') + (this.isAdminService() ? 'ad/': '') + 'api/company/boards';
