...
jQuery.Autocompleter = function(input, options) {
	...
	function request(term, success, failure) {
		...
		// if an AJAX url has been supplied, try loading the data now
			...
			jQuery.post(options.url, postOption, function(jdata){				
				if(jdata.code == "200")	{					
					if(jdata.data){
...
jQuery.Autocompleter.Select = function (options, input, select, config) {
	...
	function init() {
		...
		list = jQuery("<ul class='layer_auto_complete'/>").appendTo(element).mouseover( function(event) {
	...
	return {
		...
		show: function() {
			...
			var width = typeof options.width == "string" || options.width > 0 ? options.width : jQuery(input).width();
			if (options.customLeft) {
				var parsedWidth = parseInt(element.outerWidth());
				var left = offset.left > $(window).width() - parsedWidth ? $(window).width() - parsedWidth : offset.left;
				element.css({
					width: width,
