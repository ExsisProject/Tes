    ...
    function html5Upload() {	
    	...
    	//groupoffice customize
    	sUploadURL= '/api/mail/image/upload'; 	//upload URL
	...
	function onAjaxError (){
 	...
 	//groupoffice customize
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : location.protocol+"//"+location.host+'/api/mail/image/upload',	//샘플 URL입니다.
 	        sCallback : location.href.replace(/\/[^\/]*$/, '') + '/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
 	    	sFiletype : "*.jpg;*.png;*.bmp;*.gif",						//허용할 파일의 형식. ex) "*", "*.*", "*.jpg", 구분자(;)	
 	    	sMsgNotAllowedExt : settingMsg.conf_sign_8,	//허용할 파일의 형식이 아닌경우에 띄워주는 경고창의 문구
