function createSEditor2(elIRField, htParams, elSeAppContainer){
	...
	oEditor.registerPlugin(new nhn.husky.MessageManager(oMessageMap,htParams.locale));
	...
	oEditor.registerPlugin(new nhn.husky.SE2M_Accessibility(elAppContainer,htParams.locale));				// 에디터내의 웹접근성 관련 기능모음 플러그인 
	
	// 양식편집기용 파레트 추가
	if (htParams.bUsePallet) {
		oEditor.registerPlugin(new nhn.husky.SE_InitPallet(htParams.bUsePallet, htParams.bUseApprovalType, htParams.approvalInfo)); //에디터 팔레트 초기화 플러그인
		oEditor.registerPlugin(new nhn.husky.SE_FormEditor(elAppContainer, htParams.bUsePallet)); //편집기 플러그인
	}
