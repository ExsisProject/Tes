...
nhn.husky.SE_EditingAreaManager = jindo.$Class({
	...
	$BEFORE_MSG_APP_READY : function(msg){
		...
		/**
		 * GO-11992 [메일] 메일을 쓰면서 서명첨부를 하면 에디터에 바로 표시되도록 개선해 주십시요.
		 */
		this.oApp.exec("ADD_APP_PROPERTY", ["setSignArea", this.setSignArea]);
	 ...
	 * GO-11992 [메일] 메일을 쓰면서 서명첨부를 하면 에디터에 바로 표시되도록 개선해 주십시요.
	 */
	setSignArea : function(signData){
		var content = this.oApp.getContents();
		if(content.indexOf("<!--sign Area start-->") != -1 && content.indexOf("<!--sign Area end-->") != -1) {
			...
			content = prevContent + "<!--sign Area start-->" +  signData + "<!--sign Area end-->" + nextContent;
			this.oApp.setContents(content);
		}else{
			var sign = "<!--sign Area start-->" + signData + "<!--sign Area end-->";
			$Agent().navigator().ie ? sign = ["<P>&nbsp;</P><P>&nbsp;</P>", sign].join("") : sign = ["<P><BR></P><P><BR></P>", sign].join("");
			this.oApp.exec("PASTE_HTML", [sign]);
		}		
	},
	...
	$ON_LOAD_CONTENTS_FIELD : function(bDontAddUndo){
		...
		//groupoffice customize
		if(!sContentsFieldValue){
			sContentsFieldValue = "";
		}
...
//		el.style.width = this.elEditingAreaContainer.style.width || (this.elEditingAreaContainer.offsetWidth+"px");
	...
	getContents : function(){
		...
		/**
		 * GO-16709 [스마트에디터] 기본 글꼴 지정 가능하도록 개선
		 */ 
		var fontFamily = this.oApp.getWYSIWYGDocument().body.style.fontFamily;
		var fontSize = this.oApp.getWYSIWYGDocument().body.style.fontSize;
		...
		// groupoffice customize GO-16709 
		/*디폴트 폰트,사이즈 start*/
		if(fontFamily != "" || fontSize != ""){
			sContents = "<div style='font-family:"+fontFamily+";font-size:"+fontSize+";'>" + sContents + "</div>";
		}
		/*디폴트 폰트,사이즈 end*/
...
//	iMinUndoInterval : 50,
	...
	$ON_EVENT_EDITING_AREA_PASTE_DELAY : function(weEvent) {	
		...
		this._DAOUOFFICE_IE_TABLE_CLASS_CONVERTER();
	},

	/**
	 * DAOUOFFICE 커스텀. IE에서 붙여넣기한 TABLE에 대하여 __se_tbl CLASS를 추가해준다.
	 */
	_DAOUOFFICE_IE_TABLE_CLASS_CONVERTER: function() {
		var oNavigator = this.oApp.oNavigator;

		if(!oNavigator.ie){
			return;
		}
		var table = jindo.$$('table[class!="__se_tbl"]', this.oApp.getWYSIWYGDocument().body, { oneTimeOffCache:true });
		jindo.$A(table).forEach(function(value) {
			jindo.$Element(value).addClass('__se_tbl');
		});
	},
...
nhn.husky.SE_EditingAreaVerticalResizer = jindo.$Class({
	...
	$ON_MSG_APP_READY : function(){
		...
		//groupoffice customize
		if(!!this.welNoticeLayer && !Number(jindo.$Cookie().get(this.sCookieNotice)) && this.isUseVerticalResizer()){
			this.welNoticeLayer.delegate("click", "BUTTON.bt_clse", jindo.$Fn(this._closeNotice, this).bind());
			this.welNoticeLayer.show();
		}
...
nhn.husky.SE_PasteHandler = jindo.$Class({
	...
	_filterPastedContents : function(sOriginalContent, bUseTableFilter){
		...
		// groupoffice customize DOCUSTOM-4095
		goCommentTag = /<!--[^*]*-->/,
			...
			while(sOriginalContent != ""){
				...
				if(rxOpeningAndClosingTag.test(sOriginalContent)){ // <tagName />
					...
					if(sTagName === 'TABLE'){
						 ...
						 * */
                        /**
                         * [GO-20132] DaouOffice Custom -
                         * 스마트 에디터에서 막아둔 부분을 사이트 요청으로 인해 해제한다. 어떤 문제가 생길지는 알 수 없음.
                         */
						if(nTableDepth === 0){
							if(sClassAttr){
								if(sClassAttr.indexOf('__se_tbl') === -1){
									if(rxClass_rest.test(sMatch)){ // class="className [className2...]"
										sMatch = sMatch.replace(rxClass_rest, "$1$2 __se_tbl$3");
									}else if(rxSingleClass_underIE8.test(sMatch)){ // [IE8-] class=className
										sMatch = sMatch.replace(rxSingleClass_underIE8, '$1"$2 __se_tbl"');
									}
								}
							}else{
								/**
								 * GO 엑셀 붙여넣기한 데이터, 셀 리사이즈
								 */
								sMatch = sMatch.replace(rxTableClassAdd, '$1 class="__se_tbl"');
							}
							/**
							 * GO-20576(엑셀 표 틀어짐 현상), GO-20328(엑셀 copy & paste 기능 오동작)
							 */
							sMatch = sMatch.replace(rxStyle, "$1$2" + 'word-break:break-all; ' + "$3$4");
						}
						// --[SMARTEDITORSUS-1673]
				...
				// groupoffice customize DOCUSTOM-4095 start
				else if (goCommentTag.test(sOriginalContent)) {
					sMatch = sOriginalContent.match(goCommentTag)[0];

					sResult = sMatch;

					rxApplied = goCommentTag;
				// groupoffice customize DOCUSTOM-4095 end
...
nhn.husky.SE2M_FontNameWithLayerUI = jindo.$Class({
	...
	_getFontFamilyFromLI : function(elLi){
		...
		/**
		 * GO-27391.rv // jira 번호 없고 리비전만 있음.
		 */
		if(elLi.getElementsByTagName("EM").length == 0){
			return '';
		}
	...
	$init : function(oMessageMap, sLocale){
		switch(sLocale) {
			...
			case "zh_CN" :
				...
				break;
			case "vi" :
				this.oMessageMap = oMessageMap_vi;
				break;	
			default :  // Korean
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_Hyperlink, {
	...
	$ON_APPLY_HYPERLINK : function(){
		...
		if(this._validateURL(sURL)){
			...
			// }else{
			/**
			 * GO-13901 [스마트에디터] 하이퍼링크의 target을 _blank 로 수정
			 */
				sTarget = "_blank"; 
			//}
...
var oMessageMap = {
	...
	'SE2M_EditingModeChanger.confirmTextMode' : '텍스트 모드로 전환하면 작성된 내용은 유지되나, \n\n글꼴 등의 편집효과와 이미지 등의 첨부내용이 모두 사라지게 됩니다.\n\n전환하시겠습니까?',
	'SE2M_FontNameWithLayerUI.sSampleText' : '가나다라',
	'SE_Approval.DupError' : '해당컴포넌트는 템플릿당 한개씩만 사용가능합니다.',
	'SE_Approval.OfficialError' : '해당컴포넌트를 사용할수 없습니다.'
};
...
var oMessageMap_ja_JP = {
	...
	'SE_Hyperlink.invalidURL' : '入力されたURLが正しくありません。',
	'SE_FindReplace.keywordMissing' : '検索する文字例を入力してください。',
	'SE_FindReplace.keywordNotFound' : '検索結果がありません。',
	'SE_FindReplace.replaceAllResultP1' : '一致する文字列 ',
	'SE_FindReplace.replaceAllResultP2' : '件が置換されました。',
	'SE_FindReplace.notSupportedBrowser' : '現在使用しているブラウザではご利用できない機能です。',
	'SE_FindReplace.replaceKeywordNotFound' : '置換する項目がありません。',
	'SE_LineHeight.invalidLineHeight' : '無効な値です。',
	'SE_Footnote.defaultText' : '脚注の内容を入力してください',
	'SE.failedToLoadFlash' : 'フラッシュが禁止されており、この機能を使用することができません。',
	'SE2M_EditingModeChanger.confirmTextMode' : 'TEXTモードに切り替えると、作成した内容は保持されますが、\n\n フォントの効果や挿入したイメージなどは消えます。\n\n 編集モードを変更しますか？',
	'SE2M_FontNameWithLayerUI.sSampleText' : 'ABCD',
	'SE_Approval.DupError' : 'このコンポーネントはテンプレートごとに一つずつしか使用できます。',
	'SE_Approval.OfficialError' : '해당컴포넌트를 사용할수 없습니다.'	
};

var oMessageMap_en_US = {
	'SE_EditingAreaManager.onExit' : 'Successfully changed.',
	'SE_Color.invalidColorCode' : 'Please enter the color code correctly. \n\n eg. #000000, #FF0000, #FFFFFF, #ffffff, ffffff',
	'SE_Hyperlink.invalidURL' : 'The URL you\'ve entered is not correct.',
	'SE_FindReplace.keywordMissing' : 'Please enter a word to find.',
	'SE_FindReplace.keywordNotFound' : 'No words found.',
	'SE_FindReplace.replaceAllResultP1' : 'In total ',
	'SE_FindReplace.replaceAllResultP2' : 'matches were changed.',
	'SE_FindReplace.notSupportedBrowser' : 'You cannot use the feature on the browser you\'re using.\n\nSorry for the inconvenience.',
	'SE_FindReplace.replaceKeywordNotFound' : 'No words changed.',
	'SE_LineHeight.invalidLineHeight' : 'The value is invalid.',
	'SE_Footnote.defaultText' : 'Please enter a footnote.',
	'SE.failedToLoadFlash' : 'You cannot use this feature because Flash is not enabled.',
	'SE2M_EditingModeChanger.confirmTextMode' : 'If you switch the edit mode to Text Mode, contents will remain but font effects and attachments like images will discarded.\n\nAre you sure to continue?',
	'SE2M_FontNameWithLayerUI.sSampleText' : 'ABCD',
	'SE_Approval.DupError' : 'The component can be used one at a time.',
	'SE_Approval.OfficialError' : '해당컴포넌트를 사용할수 없습니다.'
};

var oMessageMap_zh_CN = {
	'SE_EditingAreaManager.onExit' : '内容已更改。',
	'SE_Color.invalidColorCode' : '请输入正确的颜色代码。 \ n \ n例）＃000000，＃000000，＃FFFFFF，＃FFFFFF FFFFFF',
	'SE_Hyperlink.invalidURL' : '输入的网址不正确。',
	'SE_FindReplace.keywordMissing' : '请输入搜索单词。',
	'SE_FindReplace.keywordNotFound' : '没有搜索结果。',
	'SE_FindReplace.replaceAllResultP1' : '匹配的字符串 ',
	'SE_FindReplace.replaceAllResultP2' : '件已切换。',
	'SE_FindReplace.notSupportedBrowser' : '目前正在使用的浏览器，无法使用该功能。',
	'SE_FindReplace.replaceKeywordNotFound' : '没有替换的单词',
	'SE_LineHeight.invalidLineHeight' : '该值是无效的。',
	'SE_Footnote.defaultText' : '请输入注脚内容。',
	'SE.failedToLoadFlash' : '因为Flash已被禁止，所以无法使用此功能。',
	'SE2M_EditingModeChanger.confirmTextMode' : '如果切换到TEXT模式，创建的内容虽然能保留，\n \n但字体和插入的图像将消失。\n \n确定要切换模式吗？',
	'SE2M_FontNameWithLayerUI.sSampleText' : 'ABCD',
	'SE_Approval.DupError' : '组件上的各模板只可以使用一个。',
	'SE_Approval.OfficialError' : '해당컴포넌트를 사용할수 없습니다.'
};
...
var oMessageMap_zh_TW = {
	...
	'SE_FindReplace.replaceAllResultP1' : '匹配的字符串 ',
	'SE_FindReplace.replaceAllResultP2' : '件已切换。',
	'SE_FindReplace.notSupportedBrowser' : '目前正在使用的瀏覽器，無法使用該功能。',
	'SE_FindReplace.replaceKeywordNotFound' : '没有替换的单词',
	'SE_LineHeight.invalidLineHeight' : '该值是无效的。',
	'SE_Footnote.defaultText' : '请输入注脚内容。',
	'SE.failedToLoadFlash' : '因为Flash已被禁止，所以无法使用此功能。',
	'SE2M_EditingModeChanger.confirmTextMode' : '如果切換到TEXT模式，創建的內容雖然能保留，\n \n但字體和插入的圖像將消失。 \n \n確定要切換模式嗎？',
	'SE2M_FontNameWithLayerUI.sSampleText' : 'ABCD',
	'SE_Approval.DupError' : '組件上的各模板只可以使用一個。',
	'SE_Approval.OfficialError' : '해당컴포넌트를 사용할수 없습니다.'
};
var oMessageMap_vi = {
	'SE_EditingAreaManager.onExit' : 'Nội dung đã được thay đổi.',
	'SE_Color.invalidColorCode' : 'Vui lòng nhập mã màu đúng. \n\n Ví dụ) #000000, #FF0000, #FFFFFF, #ffffff, ffffff',
	'SE_Hyperlink.invalidURL' : 'Nhập URL không đúng.',
	'SE_FindReplace.keywordMissing' : 'Vui lòng nhập từ muốn tìm.',
	'SE_FindReplace.keywordNotFound' : 'Không có từ muốn tìm.',
	'SE_FindReplace.replaceAllResultP1' : 'Tổng số nội dung đồng nhất ',
	'SE_FindReplace.replaceAllResultP2' : 'nội dung đã được đổi.',
	'SE_FindReplace.notSupportedBrowser' : 'Chức năng không thể sử dụng trong trình duyệt hiện tại.\n\nXin lỗi quý khách.',
	'SE_FindReplace.replaceKeywordNotFound' : 'Không có từ muốn đổi',
	'SE_LineHeight.invalidLineHeight' : 'Giá trị lỗi.',
	'SE_Footnote.defaultText' : 'Vui lòng nhập nội dung chú thích',
	'SE.failedToLoadFlash' : 'Flash bị chặn nên không thể sử dụng chức năng này.',
	'SE2M_EditingModeChanger.confirmTextMode' : 'Nếu chuyển chế độ văn bản thì các nội dung đãsoạn thảosẽ giữ nguyên,\n\n nhưng các nội dung đính kèm như hiệu ứngphôngchữ sẽ mấthết.\n\nBạn có muốn chuyển đổi không?',
	'SE2M_FontNameWithLayerUI.sSampleText' : 'ABCD',
	'SE_Approval.DupError' : 'Một component (bộ phận) chỉ được sử dụng cho một template (bản mẫu).',
	'SE_Approval.OfficialError' : '해당컴포넌트를 사용할수 없습니다.'
};