...
function SGJ_base64EncodeFile(input, output)
{
	var SG = document.getElementById(object_id);
	...
	{
		setErrorCode( "PARAMETER_NULL" );
		setErrorFunction( "SGJ_base64EncodeFile()" );
		return "";
	}
	
	var isok = SG.base64EncodeFile(input, output);
	return isok;
}
...
function SGJ_base64DecodeFile(input, output)
{
	var SG = document.getElementById(object_id);
	...
	{
		setErrorCode( "PARAMETER_NULL" );
		setErrorFunction( "SGJ_base64DecodeFile()" );
		return "";
	}
	
	var isok = SG.base64DecodeFile(input, output);
	return isok;
...
function SGJ_genPersonInfoReq(strUserID, userAgree, agreeInfo)
{
	clearError();
	var SG = document.getElementById(object_id);
	if ( isNull(strUserID) || isNull(userAgree) || isNull(agreeInfo))
	{
		setErrorCode( "PARAMETER_NULL" );
		setErrorFunction( "SGJ_genPersonInfoReq()" );
		return "";
	}
	
	var msg = SG.genPersonInfoReq(strUserID, userAgree, agreeInfo);
	if ( isNull(msg))
	{
		setErrorCode( "INVALID_RETURN_VALUE" );
		setErrorMsg(SGJ_getErrorMsg());
...
//jre 버전 찾기
function SGJ_getJreVersion()
{
	clearError();
	var SG = document.getElementById(object_id);
	var msg = SG.getJreVersion();
	if ( isNull(msg))
	{
		setErrorCode( "INVALID_RETURN_VALUE" );
		setErrorMsg(SGJ_getErrorMsg());
		setErrorFunction( "SGJ_getJreVersion()" );
		return "";
	}
	return msg;
}
